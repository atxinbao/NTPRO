...
当前 Backtest 可以选择 `<workspace>/catalog` 中经 Rust 校验的本地 Parquet QuoteTick
数据，并把品种、完整时间范围和 SHA-256 指纹冻结到 Run。内置确定性数据继续用于产品
流程和可复现验证，不代表真实市场研究或收益证明。Live 已有独立准入与受控技术路径，
但真实资金端到端验收仍需 owner 单独授权，不是 Usable Product v1 的完成前提。
