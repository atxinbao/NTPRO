...
REPO="${NTPRO_V301_RELEASE_REPO:-atxinbao/NTPRO}"
RELEASE_VERSION="${NTPRO_V301_RELEASE_VERSION:-v0.30.1}"
RELEASE_TAG="${NTPRO_V301_RELEASE_TAG:-ntpro-rust-only-v0.30.1}"
RELEASE_NAME="${NTPRO_V301_RELEASE_NAME:-NTPRO Rust-only v0.30.1}"
BASE_RELEASE_TAG="${NTPRO_V301_BASE_RELEASE_TAG:-ntpro-rust-only-v0.30.0}"
MANIFEST_PATH="${NTPRO_V301_RELEASE_MANIFEST:-docs/rust-cutover/release/v0_30_1_release_manifest.json}"
RELEASE_NOTES_PATH="${NTPRO_V301_RELEASE_NOTES:-docs/rust-cutover/release/v0_30_1_release_notes.md}"
READINESS_REPORT_PATH="${NTPRO_V301_READINESS_REPORT:-docs/rust-cutover/release/v0_30_1_readiness_report.md}"
CLOSEOUT_PATH="${NTPRO_V301_CLOSEOUT:-docs/rust-cutover/release/v0_30_1_release_closeout_evidence.md}"
CURRENT_ISSUE="${NTPRO_V301_CURRENT_ISSUE:-1005}"
MILESTONE_TITLE="${NTPRO_V301_MILESTONE_TITLE:-v0.30.1}"

fail() {
  echo "v30.1 release gate failed: $*" >&2
  exit 1
}
...
for path in \
  ...
  scripts/ai/verify_v30_1_strict_provenance.sh; do
  require_file "$path"
done
...
for marker in \
  ...
  "scripts/ai/verify_v30_1_v31_start_gate.sh"; do
...
for marker in \
  ...
  "source-controlled closeout evidence = docs/rust-cutover/release/v0_30_1_release_closeout_evidence.md"; do
...
scripts/ai/verify_v30_1_release_closeout_evidence.sh >/dev/null
scripts/ai/verify_v30_1_post_publication_closeout_gate.sh source >/dev/null
scripts/ai/verify_v30_1_v31_start_gate.sh source >/dev/null
NTPRO_RELEASE_GATE=0 scripts/ai/verify_v30_release_gates.sh >/dev/null
NTPRO_RELEASE_GATE=0 NTPRO_RELEASE_STRICT_REQUIRE_HEAD_TAG=0 scripts/ai/verify_v30_strict_provenance.sh >/dev/null
...
def validate(candidate: dict) -> None:
    require(candidate.get("schema_version") == "ntpro.v301_patch_release_manifest.v1", "manifest schema mismatch")
    require(candidate.get("task_id") == "V301-007", "manifest task mismatch")
    require(candidate.get("product_version") == "v0.30.1", "manifest product version mismatch")
    ...
    require(planned.get("tag") == "ntpro-rust-only-v0.30.1", "planned release tag mismatch")
    evidence = candidate.get("v301_evidence") or []
    require(len(evidence) == 7, "V301 evidence count mismatch")
    for item in evidence:
        task_id = item.get("task_id")
        require(expected.get(task_id) == item.get("issue"), f"V301 issue mismatch: {task_id}")
        require(Path(item.get("path", "")).is_file(), f"missing V301 evidence: {item}")
    scope = candidate.get("release_scope") or {}
    require(scope.get("exact_milestone_issue_numbers") == list(expected.values()), "V301 exact issue numbers mismatch")
    require(scope.get("exact_milestone_issue_set") == "#999-#1005", "V301 exact issue set mismatch")
    ...
    require(requirements.get("all_v301_issues_closed_required") is True, "V301 closeout requirement missing")
    ...
    require(requirements.get("v0_31_start_gate_fails_without_v301_release_evidence") is True, "v31 blocker missing")
    next_tracks = candidate.get("next_tracks") or {}
    require(next_tracks.get("capability") == "v0.31.0", "next capability mismatch")
    require(next_tracks.get("start_gate") == "blocked_until_v301_release_evidence_published", "next start gate mismatch")
    require(next_tracks.get("implementation_started") is False, "v31 implementation must not start")
...
missing_evidence["v301_evidence"] = missing_evidence["v301_evidence"][:-1]
try:
    validate(missing_evidence)
except SystemExit:
    pass
else:
    raise SystemExit("negative self-test unexpectedly allowed missing V301 evidence")
PY
...
expected = [999, 1000, 1001, 1002, 1003, 1004, 1005]
if sorted(states) != expected:
    raise SystemExit(f"V301 milestone issue scope mismatch: got={sorted(states)} expected={expected}")
...
if tag_gate:
    for issue in expected:
        if states[issue] != "CLOSED":
            raise SystemExit(f"V301 issue must be closed before tag gate: #{issue} state={states[issue]}")
else:
    for issue in expected:
        if issue == current_issue:
            continue
        if states[issue] != "CLOSED":
            raise SystemExit(f"V301 dependency issue is not closed: #{issue} state={states[issue]}")
    print(f"v301_issue_scope=pr_mode closed={closed}/7 current_issue_state={states.get(current_issue)}")
PY
fi
...
echo "v30_1_release_gates=pass release_tag=$RELEASE_TAG final_scope_issues=7 final_scope_evidence=7 negative_selftest=1"
