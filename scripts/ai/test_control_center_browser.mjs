...
if (!fs.existsSync(config))
  throw new Error(`MVP browser fixture is missing: ${config}`);

const readIfPresent = (filePath) =>
  fs.existsSync(filePath) ? fs.readFileSync(filePath, "utf8") : "";
const redactAccessTokens = (value) =>
  value.replace(/(access_token=)[^\s&]+/g, "$1[REDACTED]");
const sanitizeDiagnosticResult = (result) => ({
  ...result,
  ...(typeof result.error === "string"
    ? { error: redactAccessTokens(result.error) }
    : {}),
});
const diagnosticProbe = sanitizeDiagnosticResult({
  status: "fail",
  error:
    "goto http://127.0.0.1/control-center?access_token=probe-secret&event_id=1",
});
if (
  diagnosticProbe.error.includes("probe-secret") ||
  !diagnosticProbe.error.includes("access_token=[REDACTED]")
) {
...
const server = spawn(
  ...
  [
    ...
    String(NODE_SHUTDOWN_TIMEOUT_MS),
  ],
...
const writeDiagnostics = (result, logs) => {
  ...
  fs.writeFileSync(
    path.join(evidenceDir, "ntpro-node-stdout.log"),
    logs.nodeStdout,
  );
  fs.writeFileSync(
    path.join(evidenceDir, "ntpro-node-stderr.log"),
    logs.nodeStderr,
  );
  fs.writeFileSync(
...
try {
  ...
  while (Date.now() < deadline) {
    try {
      ...
      if (!operatorToken)
        throw new Error("operator bootstrap URL omitted access_token");
      const cookie = `ntpro_mvp_operator_access=${operatorToken}`;
      const [sharedResponse, snapshotResponse, correlationResponse] =
        await Promise.all([
      ...
      if (sharedResponse.ok && snapshotResponse.ok && correlationResponse.ok) {
        [sharedPayload, snapshotPayload, correlationPayload] =
          await Promise.all([
            sharedResponse.json(),
  ...
  if (
    ...
    !snapshotPayload ||
    !correlationPayload ||
    !institutionAccessUrl ||
    !operatorAccessUrl
  ) {
    throw new Error(
      `control center APIs and role bootstrap did not become ready:\n${redactAccessTokens(serverLog.join(""))}`,
    );
  }
  if (process.env.NTPRO_BROWSER_FORCE_BOOTSTRAP_DIAGNOSTIC_FAILURE === "1") {
    throw new Error(
      `forced bootstrap diagnostic failure: ${operatorAccessUrl}`,
    );
  }
  const unauthorized = await fetch(`${baseUrl}/control-center`, {
    redirect: "manual",
  });
  if (unauthorized.status !== 403)
    throw new Error(
      `unauthorized control center expected 403, got ${unauthorized.status}`,
    );
  const operatorToken = operatorAccessUrl.searchParams.get("access_token");
  ...
  if (wrongRole.status !== 403)
    throw new Error(
      `operator role reached institution workbench: ${wrongRole.status}`,
    );
  const institutionToken =
    institutionAccessUrl.searchParams.get("access_token");
  ...
  const unauthorizedAction = await fetch(lifecycleActionPath, {
    method: "POST",
    redirect: "manual",
  });
  if (unauthorizedAction.status !== 403)
    throw new Error(
      `unauthorized lifecycle action expected 403, got ${unauthorizedAction.status}`,
    );
  ...
  if (institutionAction.status !== 403)
    throw new Error(
      `institution role reached lifecycle action: ${institutionAction.status}`,
    );
  const operatorCookie = `ntpro_mvp_operator_access=${operatorToken}`;
  const lifecycleGet = await fetch(lifecycleActionPath, {
    headers: { cookie: operatorCookie },
    redirect: "manual",
  });
  if (lifecycleGet.status !== 405)
    throw new Error(`lifecycle GET expected 405, got ${lifecycleGet.status}`);
  const unsupportedAction = await fetch(
    `${baseUrl}/api/mvp/v1/control-center/nodes/mvp-node-001/actions/pause`,
    {
      method: "POST",
      headers: { cookie: operatorCookie },
      redirect: "manual",
  ...
  if (unsupportedAction.status !== 404)
    throw new Error(
      `unsupported lifecycle action expected 404, got ${unsupportedAction.status}`,
    );
  ...
  for (const forbidden of [
    ...
    if (serializedSnapshot.includes(`\"${forbidden}\"`))
      throw new Error(`operational API exposed forbidden field: ${forbidden}`);
  }
  ...
  for (const forbidden of [
    ...
    if (serializedCorrelation.includes(`\"${forbidden}\"`))
      throw new Error(
        `event correlation API exposed forbidden field: ${forbidden}`,
      );
  }
  ...
  page.on("console", (message) => {
    if (message.type() === "error" && scenario !== "ops_http_error")
  ...
  const waitForTitle = (title) =>
    page.waitForFunction(
      (expected) =>
        document.getElementById("connection-title")?.textContent === expected,
      title,
    );
  const refreshPage = async (expectedTitle) => {
    ...
    if (state.title !== expectedTitle) {
      throw new Error(
        `refresh expected ${expectedTitle}, got ${state.title}: ${state.detail}`,
      );
    }
  };
  const assertCleared = async (name) => {
    ...
    if (
      state.node !== "节点未加载" ||
      !state.components?.includes("旧数据已清空") ||
      !state.actions?.includes("操作不可用")
    ) {
      throw new Error(`${name} retained stale control center data`);
    }
  };

  await page.goto(institutionAccessUrl.toString(), {
    waitUntil: "networkidle",
  });
  if (new URL(page.url()).searchParams.has("access_token"))
    throw new Error("institution bootstrap token remained in browser URL");
  await page.goto(operatorAccessUrl.toString(), { waitUntil: "networkidle" });
  if (new URL(page.url()).searchParams.has("access_token"))
    throw new Error("operator bootstrap token remained in browser URL");
  await waitForTitle("共享与运维状态已对齐");
  const node = await page.locator("#context-node").textContent();
  if (!node || node === "节点未加载")
    throw new Error("valid browser contract did not render node identity");
  ...
  if (
    ...
    throw new Error(
      `control center exposed unexpected action controls: ${buttons.join(" | ")}`,
    );
  }
  ...
  if ((await startButton.isDisabled()) || !(await stopButton.isDisabled())) {
    throw new Error(
      "prepared node did not expose start-only lifecycle capability",
    );
  }
  const businessImpact = await page
    .locator("#business-impact-list")
    .textContent();
  if (!businessImpact?.includes("风险"))
    throw new Error("valid browser contract did not render business impact");
  ...
  if (!(await businessLink.isVisible()))
    throw new Error("control center did not render business impact jump");
  const cjkGlyphCheck = await page.evaluate(() => {
    ...
    return {
      rendered:
        signatures.every((signature) => /[1-9]/.test(signature)) &&
        new Set(signatures).size === signatures.length,
      reason: "pixel_signature",
    };
  });
  if (!cjkGlyphCheck.rendered)
    throw new Error(`Chinese glyph check failed: ${cjkGlyphCheck.reason}`);
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-1440.png"),
    fullPage: true,
  });

  await startButton.click();
  if ((await startButton.textContent()) !== "确认启动")
    throw new Error("start action did not require explicit confirmation");
  await startButton.click();
  await page.waitForFunction(() =>
    document
      .getElementById("lifecycle-action-result")
      ?.textContent?.includes("stopped → running"),
  );
  if (!(await startButton.isDisabled()) || (await stopButton.isDisabled())) {
    throw new Error(
      "started node did not expose stop-only lifecycle capability",
    );
  }
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-lifecycle-running.png"),
    fullPage: true,
  });

  await stopButton.click();
  if ((await stopButton.textContent()) !== "确认停止")
    throw new Error("stop action did not require explicit confirmation");
  await stopButton.click();
  await page.waitForFunction(() =>
    document
      .getElementById("lifecycle-action-result")
      ?.textContent?.includes("running → stopped"),
  );
  if ((await startButton.isDisabled()) || !(await stopButton.isDisabled())) {
    throw new Error(
      "stopped node did not restore start-only lifecycle capability",
    );
  }
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-lifecycle-stopped.png"),
    fullPage: true,
  });
  ...
  await page.waitForURL(
    (url) =>
      url.pathname === "/institution-workbench" &&
      url.searchParams.get("event_id") === eventId &&
      url.hash === "#event-correlation",
  );
  await waitForTitle("共享状态已验证");
  if (
    !(await page
      .locator("#event-correlation-panel")
      .textContent()
      .then((value) => value?.includes(eventId)))
  ) {
    throw new Error(
      "institution workbench did not preserve the correlated event",
    );
  }
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-390.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-shared-boundary-blocked.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-ops-http-error.png"),
    fullPage: true,
  });
  ...
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-event-mismatch.png"),
    fullPage: true,
  });

  scenario = "valid";
  await page.goto(
    `${baseUrl}/control-center?event_id=forged&event_id=${encodeURIComponent(eventId)}`,
    { waitUntil: "networkidle" },
  );
  await waitForTitle("控制中心已阻断");
  await assertCleared("duplicate event parameter");
  await page.screenshot({
    path: path.join(evidenceDir, "control-center-duplicate-event-blocked.png"),
    fullPage: true,
  });
  ...
  if (!serverExited()) {
    ...
    if (!(await waitForServerExit(PROCESS_SHUTDOWN_TIMEOUT_MS))) {
      recordFailure(
        new Error(
          `MVP server did not stop within ${PROCESS_SHUTDOWN_TIMEOUT_MS} ms and required SIGKILL`,
        ),
      );
      try {
        server.kill("SIGKILL");
      ...
      if (!(await waitForServerExit(FORCE_KILL_EXIT_TIMEOUT_MS)))
  ...
  if (!failure) {
    if (server.exitCode !== 0)
      recordFailure(
        new Error(`MVP server exited with code ${server.exitCode}`),
      );
    ...
    if (
      !logs.nodeStdout.includes(
        "external_venue_connection=false real_orders_submitted=false",
      )
    ) {
      recordFailure(
        new Error(
          "ntpro-node shutdown evidence did not preserve trading boundaries",
        ),
      );
    }
  }
