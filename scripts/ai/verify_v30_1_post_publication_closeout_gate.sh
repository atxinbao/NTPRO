...
MODE="${1:-source}"
REPO="${NTPRO_V301_POST_PUBLICATION_REPO:-atxinbao/NTPRO}"
RELEASE_VERSION="${NTPRO_V301_POST_PUBLICATION_VERSION:-v0.30.0}"
RELEASE_TAG="${NTPRO_V301_POST_PUBLICATION_TAG:-ntpro-rust-only-v0.30.0}"
RELEASE_NAME="${NTPRO_V301_POST_PUBLICATION_NAME:-NTPRO Rust-only v0.30.0}"
RELEASE_URL="${NTPRO_V301_POST_PUBLICATION_URL:-https://github.com/atxinbao/NTPRO/releases/tag/ntpro-rust-only-v0.30.0}"
TAG_SHA="${NTPRO_V301_POST_PUBLICATION_TAG_SHA:-0f0949156401fa6e6016c0160697e7090a6da788}"
TAG_OBJECT="${NTPRO_V301_POST_PUBLICATION_TAG_OBJECT:-e1c50b6189790322998fee9ee3d6d00e850b8c79}"
PUBLISHED_AT="${NTPRO_V301_POST_PUBLICATION_PUBLISHED_AT:-2026-07-11T05:37:06Z}"
GATE_RUN_ID="${NTPRO_V301_POST_PUBLICATION_GATE_RUN_ID:-29139384219}"
GATE_URL="${NTPRO_V301_POST_PUBLICATION_GATE_URL:-https://github.com/atxinbao/NTPRO/actions/runs/29139384219}"
GATE_COMPLETED_AT="${NTPRO_V301_POST_PUBLICATION_GATE_COMPLETED_AT:-2026-07-11T05:35:59Z}"
GATE_JOBS_TOTAL="${NTPRO_V301_POST_PUBLICATION_GATE_JOBS_TOTAL:-92}"
GATE_JOBS_SUCCESS="${NTPRO_V301_POST_PUBLICATION_GATE_JOBS_SUCCESS:-92}"
MILESTONE_NUMBER="${NTPRO_V301_POST_PUBLICATION_MILESTONE_NUMBER:-26}"
MILESTONE_TITLE="${NTPRO_V301_POST_PUBLICATION_MILESTONE_TITLE:-v0.30.0}"

MANIFEST_PATH="${NTPRO_V301_POST_PUBLICATION_MANIFEST:-docs/rust-cutover/release/v0_30_0_release_manifest.json}"
RELEASE_NOTES_PATH="${NTPRO_V301_POST_PUBLICATION_NOTES:-docs/rust-cutover/release/v0_30_0_release_notes.md}"
READINESS_PATH="${NTPRO_V301_POST_PUBLICATION_READINESS:-docs/rust-cutover/release/v0_30_0_readiness_report.md}"
CLOSEOUT_PATH="${NTPRO_V301_POST_PUBLICATION_CLOSEOUT:-docs/rust-cutover/release/v0_30_0_release_closeout_evidence.md}"
TASK_PATH="${NTPRO_V301_POST_PUBLICATION_TASK:-docs/rust-cutover/tasks/V301-003.md}"
EVIDENCE_PATH="${NTPRO_V301_POST_PUBLICATION_EVIDENCE:-docs/rust-cutover/evidence/V301-003.md}"

fail() {
  echo "v30.1 post-publication closeout gate failed: $*" >&2
  exit 1
}
...
def validate(candidate: dict, notes_text: str, readiness_text: str, closeout: str) -> None:
    ...
    for label, text in {"release notes": notes_text, "readiness report": readiness_text}.items():
        ...
        require("Status: PENDING PUBLICATION" not in text, f"{label} still uses pending publication status")
        ...
        require_contains(text, "published release closeout evidence = docs/rust-cutover/release/v0_30_0_release_closeout_evidence.md", label)
    ...
    require(post_pub.get("status") == "source_controlled_closeout_recorded", "post-publication status mismatch")
    require(post_pub.get("source_controlled_closeout_evidence") is True, "source-controlled closeout evidence missing")
    require(post_pub.get("source_controlled_closeout_evidence_path") == os.environ["CLOSEOUT_PATH"], "source closeout path mismatch")
    require(post_pub.get("generated_evidence_is_sole_proof") is False, "generated evidence must not be sole proof")
    require(post_pub.get("release_gate_run_id") == int(os.environ["GATE_RUN_ID"]), "post-publication gate run mismatch")
    require(post_pub.get("release_gate_status") == "completed", "post-publication gate status mismatch")
    require(post_pub.get("release_gate_conclusion") == "success", "post-publication gate conclusion mismatch")
    require(post_pub.get("release_gate_workflow_name") == "Rust Cutover Release Gate", "post-publication gate workflow mismatch")
    require(post_pub.get("release_gate_head_sha") == os.environ["TAG_SHA"], "post-publication gate head SHA mismatch")
    require(post_pub.get("release_gate_completed_at") == os.environ["GATE_COMPLETED_AT"], "post-publication gate completed_at mismatch")
    require(post_pub.get("published_after_hosted_gate") is True, "published-after-gate marker missing")
    require(post_pub.get("current_release_publish_after_gate_binding") == "pass", "current binding mismatch")
    ...
    require(release.get("draft") is False, "closeout release must not be draft")
    require(release.get("prerelease") is False, "closeout release must not be prerelease")
    require(release.get("published_at") == os.environ["PUBLISHED_AT"], "closeout published_at mismatch")
    ...
    require(milestone.get("title") == os.environ["MILESTONE_TITLE"], "milestone title mismatch")
    ...
    require(milestone.get("exact_issue_numbers") == list(range(969, 981)), "exact issue numbers mismatch")
    ...
    require(closeout_gate.get("task_id") == "V301-003", "post-publication gate task mismatch")
    require(closeout_gate.get("issue") == 1001, "post-publication gate issue mismatch")
    require(closeout_gate.get("gate") == "scripts/ai/verify_v30_1_post_publication_closeout_gate.sh source", "gate command mismatch")
    require(closeout_gate.get("release_tag") == os.environ["RELEASE_TAG"], "gate release tag mismatch")
    ...
    require(closeout_gate.get("requires_published_github_release") is True, "published release requirement missing")
    require(closeout_gate.get("requires_published_release_field") is True, "published_release field requirement missing")
    require(closeout_gate.get("requires_post_publication_closeout_field") is True, "post_publication_closeout field requirement missing")
    ...
    require(closeout_gate.get("requires_source_controlled_closeout_evidence") is True, "source closeout requirement missing")
...
for marker in [
    ...
    "published_release",
    "post_publication_closeout",
    "backend go-live;",
    "product-grade live trading claim.",
]:
    require_contains(task_text, marker, "V301-003 task")
...
for marker in [
    ...
    require_contains(evidence_text, marker, "V301-003 evidence")

if os.environ.get("NTPRO_V301_POST_PUBLICATION_SELFTEST", "1") == "1":
    def generated_only(m: dict) -> None:
        m["post_publication_closeout"]["generated_evidence_is_sole_proof"] = True
        m["post_release_closeout"]["publication_evidence"]["generated_publication_evidence_sole_proof_allowed"] = True

    manifest_mutations = {
        "release_gate_ready_only": lambda m: (m.update({"release_status": "release_gate_ready"}), m.pop("published_release", None), m.pop("post_publication_closeout", None), m.pop("post_release_closeout", None)),
        "generated_evidence_only": generated_only,
        "missing_published_release": lambda m: m.pop("published_release", None),
        "missing_post_publication_closeout": lambda m: m.pop("post_publication_closeout", None),
        "missing_closeout_gate": lambda m: m.pop("post_publication_closeout_gate", None),
        "failed_hosted_gate": lambda m: (m["post_release_closeout"]["hosted_release_gate"].update({"conclusion": "failure"}), m["post_publication_closeout"].update({"release_gate_conclusion": "failure"})),
        "published_before_gate": lambda m: (m["published_release"].update({"published_at": "2026-07-11T05:00:00Z"}), m["post_release_closeout"]["github_release"].update({"published_at": "2026-07-11T05:00:00Z"})),
    ...
    for label, bad_notes, bad_readiness in [
        ...
        ("notes_pending_publication", notes.replace("Status: RELEASED", "Status: PENDING PUBLICATION", 1), readiness),
    ]:
        try:
...
echo "v30_1_post_publication_closeout_gate status=ok mode=$MODE release_tag=$RELEASE_TAG release_gate_run=$GATE_RUN_ID source_tree_plus_github_remote=true generated_evidence_only_rejected=true release_gate_ready_only_rejected=true published_release_required=true post_publication_closeout_required=true"
