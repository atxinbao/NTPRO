...
PRODUCT_VERSION="${NTPRO_V211_PRODUCT_VERSION:-v0.21.1}"
RELEASE_TAG="${NTPRO_V211_RELEASE_TAG:-ntpro-rust-only-v0.21.1}"
EXPECTED_RELEASE_TAG="ntpro-rust-only-v0.21.1"
MANIFEST_PATH="${NTPRO_V211_STRICT_MANIFEST:-$ROOT_DIR/target/ntpro-v211/v0_21_1_strict_release_manifest.json}"
RELEASE_MANIFEST_PATH="${NTPRO_V211_RELEASE_MANIFEST:-$ROOT_DIR/docs/rust-cutover/release/v0_21_1_release_manifest.json}"
RELEASE_NOTES_PATH="${NTPRO_V211_RELEASE_NOTES:-$ROOT_DIR/docs/rust-cutover/release/v0_21_1_release_notes.md}"
READINESS_REPORT_PATH="${NTPRO_V211_READINESS_REPORT:-$ROOT_DIR/docs/rust-cutover/release/v0_21_1_readiness_report.md}"
BASE_MANIFEST_PATH="${NTPRO_V211_BASE_RELEASE_MANIFEST:-$ROOT_DIR/docs/rust-cutover/release/v0_21_0_release_manifest.json}"
VERIFY_ONLY="${NTPRO_V211_STRICT_VERIFY_ONLY:-0}"

fail() {
  echo "v21.1 strict release provenance drift: $*" >&2
  exit 1
}
...
[[ "$RELEASE_TAG" == "$EXPECTED_RELEASE_TAG" ]] || fail "v21.1 strict release tag must be $EXPECTED_RELEASE_TAG, got: $RELEASE_TAG"
[[ -f "$RELEASE_MANIFEST_PATH" ]] || fail "missing v0.21.1 release manifest: $RELEASE_MANIFEST_PATH"
[[ -f "$RELEASE_NOTES_PATH" ]] || fail "missing v0.21.1 release notes: $RELEASE_NOTES_PATH"
[[ -f "$READINESS_REPORT_PATH" ]] || fail "missing v0.21.1 readiness report: $READINESS_REPORT_PATH"
[[ -f "$BASE_MANIFEST_PATH" ]] || fail "missing v0.21.0 release manifest: $BASE_MANIFEST_PATH"
...
require(release_manifest.get("schema_version") == "ntpro.v211_patch_release_manifest.v1", "release manifest schema mismatch")
require(release_manifest.get("task_id") == "V211-006", "release manifest task mismatch")
...
require(release_manifest.get("patch_scope") == "hardening_patch_only", "release manifest patch scope mismatch")
...
require(planned.get("name") == "NTPRO Rust-only v0.21.1", "planned release name mismatch")
require(planned.get("github_release_url") == "https://github.com/atxinbao/NTPRO/releases/tag/ntpro-rust-only-v0.21.1", "planned release URL mismatch")
...
manifest = {
    "schema_version": "ntpro.v211_strict_release_provenance_manifest.v1",
    "task_id": "V211-006",
    "target": "v21.1",
    ...
    "failure_paths": {
        ...
        "manifest_mismatch": "release manifest, notes, readiness, and base manifest hashes must match",
    },
...
require(manifest.get("schema_version") == "ntpro.v211_strict_release_provenance_manifest.v1", "strict manifest schema mismatch")
require(manifest.get("task_id") == "V211-006", "strict manifest task mismatch")
require(manifest.get("target") == "v21.1", "strict manifest target mismatch")
...
echo "v21_1_strict_provenance status=ok product_version=$PRODUCT_VERSION release_tag=$RELEASE_TAG tag_exists=$release_tag_exists source_commit=$source_commit source_tree=$source_tree source_dirty=$source_dirty cargo_version=\"$cargo_version\" rustc_version=\"$rustc_version\" release_manifest=$RELEASE_MANIFEST_PATH release_manifest_sha256=$release_manifest_sha256 manifest=$MANIFEST_PATH"
