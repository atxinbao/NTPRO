...
for path in python nautilus_trader crates/pyo3 build.py pyproject.toml uv.lock .venv venv; do
  if [ -e "$path" ]; then
...
active_paths=()
for p in Cargo.toml Cargo.lock crates Makefile setup.py setup.cfg; do
  [ -e "$p" ] && active_paths+=("$p")
done
