...
const nautilusBin = path.resolve(
  process.env.NTPRO_NAUTILUS_BIN || "target/debug/nautilus",
);
const nodeBin = path.resolve(
  process.env.NTPRO_NODE_BIN || "target/debug/ntpro-node",
);
const nodeConfig = path.resolve(
  process.env.NTPRO_MVP_CONFIG || "configs/nodes/btc-ema-shadow.toml",
);
const productBacktestConfig = path.resolve(
  "configs/backtests/ema-cross-btcusdt-product.toml",
);
const root = fs.mkdtempSync(
  path.join(os.tmpdir(), "ntpro-mvp-012-fault-matrix-"),
);
...
const serverShutdownTimeoutMs = 30_000;
...
const redact = (value) =>
  String(value)
    ...
    .replace(
      /(ntpro_mvp_(?:institution|operator)_access=)[^;\s]+/g,
      "$1[REDACTED]",
    );
const serverExited = (server) =>
...
const bootstrapCookie = async (accessUrl, expectedPath, expectedCookie) => {
  ...
  assert(
    response.status === 303,
    `${expectedPath} bootstrap expected 303, got ${response.status}`,
  );
  assert(
    response.headers.get("location") === expectedPath,
    `${expectedPath} bootstrap location mismatch`,
  );
  const setCookie = response.headers.get("set-cookie");
  assert(
    setCookie?.includes("HttpOnly"),
    `${expectedPath} cookie is not HttpOnly`,
  );
  assert(
    setCookie?.includes("SameSite=Strict"),
    `${expectedPath} cookie is not SameSite=Strict`,
  );
  const cookie = setCookie.split(";", 1)[0];
  assert(
    cookie.startsWith(`${expectedCookie}=`),
    `${expectedPath} set wrong role cookie`,
  );
  return cookie;
};
const fetchJson = async (url, cookie, options = {}) => {
  ...
  try {
    ...
    throw new Error(
      `${options.method || "GET"} ${url} returned non-JSON status=${response.status}`,
    );
  }
  return { status: response.status, body };
};
const createAndStartDemoRun = async (session) => {
  const created = await fetchJson(
    `${session.baseUrl}/api/product/v1/demo-runs`,
    session.institutionCookie,
    {
      method: "POST",
  ...
  assert(
    created.status === 201,
    `${session.name} Demo create expected 201, got ${created.status} ${JSON.stringify(created.body)}`,
  );
  const demoRunId = created.body.data?.run_id;
  assert(demoRunId, `${session.name} Demo create omitted run_id`);
  assert(
    created.body.data?.lifecycle === "created",
    `${session.name} Demo did not start as created`,
  );
  const started = await fetchJson(
    `${session.baseUrl}/api/product/v1/demo-runs/${demoRunId}/actions`,
    session.institutionCookie,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  assert(
    started.status === 200,
    `${session.name} Demo start expected 200, got ${started.status} ${JSON.stringify(started.body)}`,
  );
  assert(
    started.body.data?.current_run?.lifecycle === "running",
    `${session.name} Demo start did not project running`,
  );
  session.demoRunIds.push(demoRunId);
  return demoRunId;
};
...
const assertSharedBoundaries = (body, label) => {
  assert(
    ...
    `${label} trading readiness opened`,
  );
  assert(
    body.boundaries?.read_only === true,
    `${label} shared API is not read-only`,
  );
  assert(
    body.status?.boundaries?.read_only_product_contract === true,
    `${label} status is not read-only`,
  );
  assertFalseFields(body.boundaries, `${label}.shared`, sharedFalseFields);
  assertFalseFields(
    body.status.boundaries,
    `${label}.status`,
    statusFalseFields,
  );
};
const assertSharedError = (response, expectedStatus, expectedCode, label) => {
  ...
  assert(
    response.body.schema_version === "ntpro.mvp_shared_status_api.error.v1",
    `${label} error schema mismatch`,
  );
  assert(
    response.body.error_code === expectedCode,
    `${label} error code mismatch`,
  );
  assert(
    response.body.read_only === true,
    `${label} error envelope is not read-only`,
  );
  assertFalseFields(response.body, `${label}.error`, errorFalseFields);
};
const assertNotHealthy = (
  body,
  label,
  expectedRuntimeStatuses = ["running", "unknown"],
) => {
...
const registryPath = (session) =>
  path.join(session.workspace, "supervisor/registry.json");
const identityPath = (session) =>
  path.join(session.workspace, "mvp/identity_contract.json");
const statusContractPath = (session) =>
  path.join(session.workspace, "mvp/status_contract.json");
...
const nodePid = (session) => {
  ...
  assert(
    ...
    `${session.name} registry omitted node PID`,
  );
  return pid;
};
...
const startSession = async (name) => {
  ...
  const server = spawn(
    ...
    [
      ...
      "crates/cli/tests/fixtures/strategy-workbench",
      "--ntpro-node-bin",
      nodeBin,
      "--startup-timeout-ms",
      "10000",
  ...
  server.stdout.on("data", (chunk) => {
    session.serverLog += chunk.toString();
  });
  ...
  const access = await waitFor(`${name} role bootstrap URLs`, 30_000, () => {
    const institution = session.serverLog.match(
      /institution_workbench_url=(\S+)/,
    )?.[1];
  ...
  await createAndStartDemoRun(session);
  await waitHealthy(session);
  nodePid(session);
  return session;
};

const waitShared = (session, description, predicate) =>
  waitFor(description, 12_000, async () => {
    const response = await fetchJson(
      `${session.baseUrl}/api/mvp/v1/status`,
      session.institutionCookie,
    );
...
const waitHealthy = (session) =>
  waitShared(session, `${session.name} healthy projection`, (response) => {
    if (response.status !== 200) {
      throw new Error(
        `${session.name} shared status returned ${response.status} ${response.body?.error_code}`,
      );
    }
    const body = response.body;
    const healthy =
      body.status?.runtime?.status === "running" &&
      body.status?.technical_health?.status === "healthy" &&
...
const stopActiveDemoRun = async (session) => {
  const record = nodeRecord(session);
  const active = Object.values(record?.run_ownership || {}).find(
    (ownership) => ownership?.terminal == null,
  );
  if (!active) return;
  const stopped = await fetchJson(
    `${session.baseUrl}/api/product/v1/demo-runs/${active.run_id}/actions`,
    session.institutionCookie,
    {
      method: "POST",
      headers: { "content-type": "application/json" },
  ...
  assert(
    stopped.status === 200,
    `${session.name} Demo cleanup stop expected 200, got ${stopped.status}`,
  );
  assert(
    stopped.body.data?.current_run?.lifecycle === "stopped",
    `${session.name} Demo cleanup did not publish stopped`,
  );
  await waitFor(`${session.name} Demo cleanup terminal anchor`, 5_000, () => {
    const current = nodeRecord(session);
    const terminal = current?.run_ownership?.[active.run_id]?.terminal;
    return current?.process?.state === "stopped" &&
...
const stopSession = async (session) => {
  ...
  await stopActiveDemoRun(session);
  session.server.kill("SIGINT");
  assert(
    await waitForServerExit(session.server, serverShutdownTimeoutMs),
    `${session.name} server did not stop after SIGINT`,
  );
  assert(
    session.server.exitCode === 0,
    `${session.name} server exited with code ${session.server.exitCode}`,
  );
};
const cleanupSession = async (session) => {
  ...
  if (!serverExited(session.server)) {
    session.server.kill("SIGINT");
    if (!(await waitForServerExit(session.server, serverShutdownTimeoutMs))) {
  ...
  for (const pid of session.observedNodePids) {
    ...
    const stopped = await waitFor(
      `cleanup of ${session.name} node ${pid}`,
      5_000,
    ...
    if (!stopped && processIsAlive(pid)) {
      ...
      await waitFor(
        `forced cleanup of ${session.name} node ${pid}`,
        5_000,
        () => (!processIsAlive(pid) ? true : undefined),
      );
    }
  ...
  assert(
    leakedPids.length === 0,
    `${session.name} leaked node PIDs: ${leakedPids.join(",")}`,
  );
...
const observeNoAutomaticRecovery = async (
  ...
  while (Date.now() - startedAt < noAutomaticRecoveryObservationMs) {
    const record = nodeRecord(session);
    assert(
      record?.process?.state === expectedProcessState,
      `${label} process state changed automatically`,
    );
    assert(
      record?.process?.pid?.value === expectedPid,
      `${label} process PID changed automatically`,
    );
    const response = await fetchJson(
      `${session.baseUrl}/api/mvp/v1/status`,
      session.institutionCookie,
  ...
  assert(
    ...
    `${label} lifecycle events changed without operator action`,
  );
  assert(
    samples >= 10,
    `${label} observation did not span enough heartbeat samples`,
  );
  return {
...
const artifactMatrix = async () => {
  ...
  results.push(
    await runFrozenArtifactFault(
      session,
      "status_missing",
      async ({ paths }) => fs.rmSync(paths.status),
      async ({ session: current }) => {
        const response = await waitShared(
          current,
    ...
    await runFrozenArtifactFault(
      ...
      async ({ session: current }) => {
        const response = await waitShared(
          ...
          (candidate) =>
            candidate.status === 200 &&
            candidate.body.status?.technical_health?.availability ===
              "missing" &&
            candidate.body.status?.technical_health?.status === "degraded",
        );
  ...
  results.push(
    await runFrozenArtifactFault(
      ...
      async ({ session: current }) => {
        ...
        assert(
          reasons.includes("node_status_timestamp_stale"),
          "stale status reason missing",
        );
        assert(
          reasons.includes("node_metrics_timestamp_stale"),
          "stale metrics reason missing",
        );
      },
    ),
  );
  results.push(
    await runFrozenArtifactFault(
      ...
      async ({ paths }) => {
        ...
        assert(
          Number.isFinite(statusGeneration) && statusGeneration > 1,
          "status generation is invalid",
        );
        metrics.generated_at = {
          availability: "available",
          value: String(statusGeneration - 1),
        };
        writeJson(paths.metrics, metrics);
      },
      async ({ session: current }) => {
        const response = await waitShared(
          ...
          (candidate) =>
            candidate.status === 200 &&
            candidate.body.status?.technical_health?.reasons?.includes(
              "status_metrics_generation_mismatch",
            ),
        );
    ...
    await runFrozenArtifactFault(
      ...
      async ({ session: current }) => {
        const response = await waitShared(
          ...
          (candidate) => candidate.status === 503,
        );
        assertSharedError(
          response,
          503,
        ...
        await waitFor(
          "missing identity local unhealthy contract",
          5_000,
          () => {
            const status = readJson(statusContractPath(current));
            return (
              status.provenance?.identity_contract_available === false &&
  ...
  results.push(
    await runFrozenArtifactFault(
      ...
      "identity_invalid",
      async ({ session: current }) =>
        fs.writeFileSync(identityPath(current), "not-json\n"),
      async ({ session: current }) => {
        const response = await waitShared(
          current,
...
const gracefulExitMatrix = async () => {
  ...
  const stopped = await waitShared(
    session,
    "external SIGTERM stopped projection",
    (response) =>
      response.status === 200 &&
  ...
  assert(
    stoppedRecord?.process?.state === "stopped",
    "SIGTERM registry process is not stopped",
  );
  assert(
    stoppedRecord?.process?.pid?.value === undefined,
    "SIGTERM registry retained a PID",
  );

  const snapshot = await waitFor(
    "external SIGTERM operational stopped projection",
    10_000,
    async () => {
      const response = await fetchJson(
      ...
      return response.status === 200 &&
        response.body.node?.lifecycle_state === "stopped"
        ? response
        : undefined;
    },
  );
  assertFalseFields(
    snapshot.body.boundaries,
    "external_sigterm.control_center",
    operationalFalseFields,
  );
  ...
  const noAutomaticRecovery = await observeNoAutomaticRecovery(
    ...
    (response) => {
      assert(
        response.status === 200,
        `external SIGTERM observation returned ${response.status}`,
      );
      assert(
        response.body.status?.runtime?.status === "stopped",
        "external SIGTERM runtime changed",
      );
      assert(
        response.body.status?.technical_health?.status === "not_running",
  ...
  const recoveredRunId = await createAndStartDemoRun(session);
  await waitHealthy(session);
  const recoveredPid = nodePid(session);
  assert(
    recoveredPid !== originalPid,
    "SIGTERM recovery reused the exited PID",
  );
  await stopSession(session);
  return {
    signal: "SIGTERM",
    ...
    recovery: "explicit_demo_create_start",
    recovered_demo_run_id: recoveredRunId,
...
const hardKillMatrix = async () => {
  ...
  const stale = await waitShared(
    session,
    "hard-kill fail-closed projection",
    (response) =>
      response.status === 200 &&
      response.body.status?.runtime?.status === "unknown" &&
      ["degraded", "unhealthy"].includes(
        response.body.status?.technical_health?.status,
      ),
  );
  assertNotHealthy(stale.body, "hard_kill", ["unknown"]);
  assert(
    stale.body.status.runtime.reasons?.includes(
      "supervisor_process_state_stale",
    ),
    "hard-kill stale reason missing",
  );
  ...
  const noAutomaticRecovery = await observeNoAutomaticRecovery(
    ...
    (response) => {
      assert(
        response.status === 200,
        `hard-kill observation returned ${response.status}`,
      );
      assertNotHealthy(response.body, "hard_kill.observation", ["unknown"]);
      assert(
        response.body.status.runtime.reasons?.includes(
          "supervisor_process_state_stale",
        ),
  ...
  const unauthenticated = await fetchJson(
    actionUrl(session, "stop"),
    undefined,
    { method: "POST" },
  );
  assert(
    unauthenticated.status === 403,
    `hard-kill unauthenticated action expected 403, got ${unauthenticated.status}`,
  );
  const institution = await fetchJson(
    actionUrl(session, "stop"),
    session.institutionCookie,
    { method: "POST" },
  );
  assert(
    institution.status === 403,
    `hard-kill institution action expected 403, got ${institution.status}`,
  );
  ...
  assert(
    lifecycleGet.status === 405,
    `hard-kill lifecycle GET expected 405, got ${lifecycleGet.status}`,
  );
  const failedStop = await fetchJson(
    actionUrl(session, "stop"),
    session.operatorCookie,
    { method: "POST" },
  );
  assert(
    failedStop.status === 500,
    `hard-kill stop expected 500, got ${failedStop.status}`,
  );
  assert(
    failedStop.body.result?.status === "failed",
    "hard-kill stop was not reported as failed",
  );
  assertFalseFields(
    failedStop.body.boundaries,
    "hard_kill.failed_stop",
    actionFalseFields,
  );

  const failedRunId = session.demoRunIds.at(-1);
  assert(failedRunId, "hard-kill session omitted its active Demo Run");
  const terminalized = await fetchJson(
    `${session.baseUrl}/api/product/v1/runs/${failedRunId}`,
    session.institutionCookie,
  );
  assert(
    terminalized.status === 503,
    `hard-kill terminal read expected 503, got ${terminalized.status}`,
  );
  assert(
    terminalized.body.error?.code === "product_source_stale",
    "hard-kill terminal read did not remain fail-closed",
  );
  await waitFor("hard-kill failed Demo terminal anchor", 5_000, () => {
    const terminal =
      nodeRecord(session)?.run_ownership?.[failedRunId]?.terminal;
    return terminal?.lifecycle === "failed" ? terminal : undefined;
  });
  ...
  assert(
    !start.error,
    `manual supervisor start failed to launch: ${start.error?.message}`,
  );
  assert(
    start.status === 0,
    `manual supervisor start exited ${start.status}: ${redact(start.stderr || start.stdout)}`,
  );
  assert(
    start.stdout.includes("supervisor.start status=ok"),
    "manual supervisor start omitted success evidence",
  );
  assert(
    start.stdout.includes("external_venue_connection=false"),
    "manual start opened external venue",
  );
  assert(
    start.stdout.includes("real_orders_submitted=false"),
    "manual start submitted real orders",
  );
  await waitHealthy(session);
  const recoveredPid = nodePid(session);
  assert(
    recoveredPid !== originalPid,
    "hard-kill recovery reused the exited PID",
  );
  await stopSession(session);
  return {
    signal: "SIGKILL",
...
try {
  assert(
    ...
    "MVP fault matrix requires Unix process signals",
  );
  ...
  assert(
    !redactionProbe.includes("fault-probe"),
    "access token redaction self-test failed",
  );
  assert(
    !redactionProbe.includes("fault-cookie"),
    "cookie redaction self-test failed",
  );
  ...
  const automaticActionObserved = [gracefulExit, hardKill].some(
    (entry) =>
      entry.no_automatic_recovery_observation.automatic_action_observed,
  );
  assert(
    !automaticActionObserved,
    "process matrix observed an automatic recovery action",
  );
  passResult = {
  ...
  const cleanupComplete = sessions.every(
    (session) => session.cleanupVerified === true,
  );
  if (!cleanupComplete) {
    failure ||= new Error(
      "one or more fault-matrix sessions did not prove node cleanup",
    );
  }
  const logs = sessions
  ...
  assert(
    !/access_token=(?!\[REDACTED\])/.test(sanitizedLog),
    "fault log contains an access token",
  );
  assert(
    !/ntpro_mvp_(institution|operator)_access=(?!\[REDACTED\])/.test(
      sanitizedLog,
    ),
  ...
  assert(
    !/access_token=(?!\[REDACTED\])/.test(serialized),
    "result.json contains an access token",
  );
  assert(
    !/ntpro_mvp_(institution|operator)_access=(?!\[REDACTED\])/.test(
      serialized,
