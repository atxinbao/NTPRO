...
PRODUCT_VERSION="${NTPRO_V281_PRODUCT_VERSION:-v0.28.1}"
RELEASE_TAG="${NTPRO_V281_RELEASE_TAG:-ntpro-rust-only-v0.28.1}"
EXPECTED_RELEASE_TAG="ntpro-rust-only-v0.28.1"
MANIFEST_PATH="${NTPRO_V281_STRICT_MANIFEST:-$ROOT_DIR/target/ntpro-v281/v0_28_1_strict_release_manifest.json}"
RELEASE_MANIFEST_PATH="${NTPRO_V281_RELEASE_MANIFEST:-$ROOT_DIR/docs/rust-cutover/release/v0_28_1_release_manifest.json}"
RELEASE_NOTES_PATH="${NTPRO_V281_RELEASE_NOTES:-$ROOT_DIR/docs/rust-cutover/release/v0_28_1_release_notes.md}"
READINESS_REPORT_PATH="${NTPRO_V281_READINESS:-$ROOT_DIR/docs/rust-cutover/release/v0_28_1_readiness_report.md}"

fail() {
  echo "v28.1 strict release provenance drift: $*" >&2
  exit 1
}

[[ "$RELEASE_TAG" == "$EXPECTED_RELEASE_TAG" ]] || fail "v28.1 strict release tag must be $EXPECTED_RELEASE_TAG, got: $RELEASE_TAG"
...
input_paths=(
  ...
  "$ROOT_DIR/docs/rust-cutover/release/v0_28_0_release_manifest.json"
  "$ROOT_DIR/docs/rust-cutover/release/v0_28_0_release_closeout_evidence.md"
  "$ROOT_DIR/docs/rust-cutover/release/v0_28_0_release_notes.md"
  "$ROOT_DIR/docs/rust-cutover/release/v0_28_0_readiness_report.md"
  "$ROOT_DIR/README.md"
  "$ROOT_DIR/ROADMAP.md"
  ...
  "$ROOT_DIR/scripts/ai/verify_v28_1_release_body_hash_normalization.sh"
  "$ROOT_DIR/scripts/ai/verify_v28_1_runtime_closed_terminology.sh"
  "$ROOT_DIR/scripts/ai/verify_v28_1_release_gates.sh"
  "$ROOT_DIR/scripts/ai/verify_v28_1_strict_provenance.sh"
...
require(release_manifest["task_id"] == "V281-008", "manifest task mismatch")
...
require(scope.get("exact_milestone_issue_numbers") == [919, 920, 921, 922, 923, 924, 925, 944], "exact milestone issue numbers mismatch")
require(scope.get("exact_milestone_issue_set") == "#919-#925, #944", "exact milestone issue set mismatch")
require(scope.get("final_release_scope_issue_count") == 8, "final release scope issue count mismatch")
require(scope.get("final_release_scope_evidence_count") == 8, "final release scope evidence count mismatch")
...
payload = {
    ...
    "v281_issue_scope": [919, 920, 921, 922, 923, 924, 925, 944],
    ...
    "post_publication_closeout": release_manifest.get("post_publication_closeout"),
    ...
    "failure_paths": {
        ...
        "open_v281_issue": "NTPRO_RELEASE_GATE=1 fails unless V281 issues #919-#925 and #944 are closed",
...
echo "v28_1_strict_provenance status=ok release_tag=$RELEASE_TAG tag_exists=$release_tag_exists source_dirty=$source_dirty manifest=${MANIFEST_PATH#$ROOT_DIR/}"
