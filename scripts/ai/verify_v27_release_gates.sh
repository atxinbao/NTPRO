...
CURRENT_ISSUE="${NTPRO_V270_CURRENT_ISSUE:-861}"
...
def validate(candidate: dict) -> None:
    ...
    require(len(evidence) == 9, "V270 evidence count mismatch")
...
if command -v gh >/dev/null 2>&1 && gh auth status >/dev/null 2>&1; then
  for issue in 853 854 855 856 857 858 859 860; do
...
if os.environ["RELEASE_GATE"] == "1" or os.environ["REQUIRE_CLOSEOUT"] == "1":
    if milestone["state"] != "closed" or milestone["open_issues"] != 0 or milestone["closed_issues"] < 9:
        raise SystemExit(f"v0.27.0 milestone must be closed with at least 9 closed issues for tag gate: {milestone}")
else:
...
echo "v27_release_gates status=ok release_tag=$RELEASE_TAG base_release=$BASE_RELEASE_TAG current_issue_state=$current_state v270_issues=8/9_closed_or_current final_scope_issues=9 negative_selftest=${NTPRO_V270_RELEASE_SELFTEST:-1}"
