...
run_response_redaction "$READY_RESPONSE" "$MISSING_FLAGS_REDACTION" >/dev/null

run_response_redaction "$READY_RESPONSE" "$READY_REDACTION" \
...
run_response_redaction "$FORBIDDEN_RESPONSE" "$FORBIDDEN_REDACTION" \
  --allow-production-mutation-response-redaction \
  --confirm-owner-approved-response-redaction \
  --confirm-no-raw-response-persistence \
  --confirm-no-headers-persistence \
...
python3 - "$MISSING_FLAGS_REDACTION" "$READY_REDACTION" "$FORBIDDEN_REDACTION" <<'PY'
import json
import sys
from pathlib import Path
...
forbidden = json.loads(Path(sys.argv[3]).read_text())
...
echo "v16_response_redaction status=ok root=$GATE_ROOT ready_response_redacted=true raw_exchange_response_recorded=false response_headers_recorded=false unrestricted_payload_recorded=false account_balances_recorded=false"
