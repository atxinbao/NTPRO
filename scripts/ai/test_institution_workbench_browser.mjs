...
const diagnosticProbe = sanitizeDiagnosticResult({
  status: "fail",
  error: "goto http://127.0.0.1/institution-workbench?access_token=probe-secret&event_id=1",
});
...
try {
  ...
  if (process.env.NTPRO_BROWSER_FORCE_BOOTSTRAP_DIAGNOSTIC_FAILURE === "1") {
    throw new Error(`forced bootstrap diagnostic failure: ${institutionAccessUrl}`);
  }
