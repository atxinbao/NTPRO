...
PRODUCT_VERSION="${NTPRO_V291_PRODUCT_VERSION:-v0.29.1}"
RELEASE_TAG="${NTPRO_V291_RELEASE_TAG:-ntpro-rust-only-v0.29.1}"
EXPECTED_RELEASE_TAG="ntpro-rust-only-v0.29.1"
MANIFEST_PATH="${NTPRO_V291_STRICT_MANIFEST:-$ROOT_DIR/target/ntpro-v291/v0_29_1_strict_release_manifest.json}"
RELEASE_MANIFEST_PATH="${NTPRO_V291_RELEASE_MANIFEST:-$ROOT_DIR/docs/rust-cutover/release/v0_29_1_release_manifest.json}"
RELEASE_NOTES_PATH="${NTPRO_V291_RELEASE_NOTES:-$ROOT_DIR/docs/rust-cutover/release/v0_29_1_release_notes.md}"
READINESS_REPORT_PATH="${NTPRO_V291_READINESS:-$ROOT_DIR/docs/rust-cutover/release/v0_29_1_readiness_report.md}"

fail() {
  echo "v29.1 strict release provenance drift: $*" >&2
  exit 1
}

[[ "$RELEASE_TAG" == "$EXPECTED_RELEASE_TAG" ]] || fail "v29.1 strict release tag must be $EXPECTED_RELEASE_TAG, got: $RELEASE_TAG"
...
input_paths=(
  ...
  "$ROOT_DIR/docs/rust-cutover/release/v0_29_0_release_manifest.json"
  "$ROOT_DIR/docs/rust-cutover/release/v0_29_0_release_closeout_evidence.md"
  ...
  "$ROOT_DIR/scripts/ai/verify_v29_1_release_gates.sh"
  "$ROOT_DIR/scripts/ai/verify_v29_1_strict_provenance.sh"
...
require(release_manifest["schema_version"] == "ntpro.v291_patch_release_manifest.v1", "manifest schema mismatch")
require(release_manifest["task_id"] == "V291-006", "manifest task mismatch")
...
require("v29.1 release gates = required" in release_notes, "release notes v29.1 gate marker missing")
require("v29.1 strict provenance = required" in readiness, "readiness strict marker missing")
scope = release_manifest.get("release_scope") or {}
require(scope.get("exact_milestone_issue_numbers") == [963, 964, 965, 966, 967, 968], "exact issue numbers mismatch")
...
payload = {
    "schema_version": "ntpro.v291_strict_release_provenance.v1",
    "task_id": "V291-006",
    "target": "v29.1",
...
echo "v29_1_strict_provenance status=ok release_tag=$RELEASE_TAG tag_exists=$release_tag_exists source_dirty=$source_dirty manifest=${MANIFEST_PATH#$ROOT_DIR/}"
