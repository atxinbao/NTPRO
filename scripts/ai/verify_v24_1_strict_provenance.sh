...
PRODUCT_VERSION="${NTPRO_V241_PRODUCT_VERSION:-v0.24.1}"
RELEASE_TAG="${NTPRO_V241_RELEASE_TAG:-ntpro-rust-only-v0.24.1}"
EXPECTED_RELEASE_TAG="ntpro-rust-only-v0.24.1"
MANIFEST_PATH="${NTPRO_V241_STRICT_MANIFEST:-$ROOT_DIR/target/ntpro-v241/v0_24_1_strict_release_manifest.json}"
RELEASE_MANIFEST_PATH="${NTPRO_V241_RELEASE_MANIFEST:-$ROOT_DIR/docs/rust-cutover/release/v0_24_1_release_manifest.json}"
RELEASE_NOTES_PATH="${NTPRO_V241_RELEASE_NOTES:-$ROOT_DIR/docs/rust-cutover/release/v0_24_1_release_notes.md}"
READINESS_REPORT_PATH="${NTPRO_V241_READINESS_REPORT:-$ROOT_DIR/docs/rust-cutover/release/v0_24_1_readiness_report.md}"
BASE_RELEASE_MANIFEST_PATH="${NTPRO_V241_BASE_RELEASE_MANIFEST:-$ROOT_DIR/docs/rust-cutover/release/v0_24_0_release_manifest.json}"
BASE_CLOSEOUT_PATH="${NTPRO_V241_BASE_CLOSEOUT:-$ROOT_DIR/docs/rust-cutover/release/v0_24_0_release_closeout_evidence.md}"
GATE_SCRIPT_PATH="${NTPRO_V241_GATE_SCRIPT:-$ROOT_DIR/scripts/ai/verify_v24_1_release_gates.sh}"
STRICT_SCRIPT_PATH="${NTPRO_V241_STRICT_SCRIPT:-$ROOT_DIR/scripts/ai/verify_v24_1_strict_provenance.sh}"
VERIFY_ONLY="${NTPRO_V241_STRICT_VERIFY_ONLY:-0}"

fail() {
  echo "v24.1 strict release provenance drift: $*" >&2
  exit 1
}

[[ "$RELEASE_TAG" == "$EXPECTED_RELEASE_TAG" ]] || fail "v24.1 strict release tag must be $EXPECTED_RELEASE_TAG, got: $RELEASE_TAG"
...
input_paths=(
  ...
  "$ROOT_DIR/docs/rust-cutover/release/v0_24_0_provenance_reconciliation.md"
  "$ROOT_DIR/docs/rust-cutover/release/v0_24_1_schema_replay_classification.md"
  "$ROOT_DIR/docs/rust-cutover/release/v0_24_1_dashboard_artifact_ingestion.md"
  "$ROOT_DIR/docs/rust-cutover/release/v0_24_1_dashboard_fixture_ref_integrity.md"
  "$ROOT_DIR/docs/rust-cutover/golden_trace/RELEASE_REPLAY_SCOPE.json"
  "$ROOT_DIR/tests/golden/v241_dashboard_order_control_artifact_ingestion.json"
)
...
require(release_manifest.get("schema_version") == "ntpro.v241_patch_release_manifest.v1", "release manifest schema mismatch")
require(release_manifest.get("task_id") == "V241-006", "release manifest task mismatch")
...
require(capability.get("patch_hardening_only") is True, "patch hardening flag mismatch")
...
if [[ "${NTPRO_V241_STRICT_REQUIRE_PUBLICATION:-0}" == "1" ]]; then
...
manifest = {
    "schema_version": "ntpro.v241_strict_release_provenance_manifest.v1",
    "task_id": "V241-006",
    "target": "v24.1",
    ...
    "v241_evidence": release_manifest.get("v241_evidence"),
    ...
    "failure_paths": {
        ...
        "missing_tag": "NTPRO_RELEASE_GATE=1 or NTPRO_RELEASE_STRICT_REQUIRE_HEAD_TAG=1 fails without the v0.24.1 release tag",
        ...
        "missing_v241_evidence": "v0.24.1 release gate fails if any V241 evidence path is missing",
        "open_operation_boundary": "v0.24.1 release gate fails if submit, mutation, adapter call, retry scheduler, Dashboard operation, or order-ticket boundary opens",
        "open_v241_issue_or_milestone": "NTPRO_RELEASE_GATE=1 fails unless V241 issues are closed and the v0.24.1 milestone is closed",
        ...
        "v25_start_without_v241_closeout": "v0.25.0 remains blocked until v0.24.1 release evidence is published",
    },
...
echo "v24_1_strict_provenance status=ok release_tag=$RELEASE_TAG tag_exists=$release_tag_exists source_dirty=$source_dirty manifest=${MANIFEST_PATH#$ROOT_DIR/}"
