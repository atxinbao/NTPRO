...
锚定的单笔 `LIMIT/GTC` 真实执行。S3-LV-008 已完成显式交易所订单对账、机构负责人申请
并由操作员确认的人工撤单，以及部分成交的累计成交量与剩余数量恢复，当前等待独立审查；
追加订单、改单、自动重试和自动恢复继续关闭。
...
FEA-001 前端架构文档
  ...
  -> S3-LV-008 交易所订单对账、人工撤单与部分成交恢复（REVIEW REQUIRED）
  -> S1-S4 三模式产品闭环
```
