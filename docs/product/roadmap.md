...
状态语义一致。机构工作台消费由 issue #1211 / PR #1212 交付；MVP-007 新增
`/control-center`，关联同一共享合同与本地只读运维投影。issue #1213 关闭且对应 PR
合并后，该阶段退出条件正式满足。

### M3：双角色最小界面（只读入口已实现，产品闭环保持开放）
...
指标、告警、来源和关闭边界视图。跨门户事件跳转、服务端角色边界和生命周期动作
产品化仍未交付，因此 M3 不随 M2 自动关闭。
