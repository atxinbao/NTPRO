...
### M2：最小只读产品接口（API 已交付，机构工作台待审查/合并）
...
交付；MVP-006 已实现 `/institution-workbench` 并只消费该接口，当前仍待审查与合并。

退出条件：机构工作台和控制中心只消费稳定投影；相同事实按角色显示，但证据编号和
状态语义一致。机构工作台消费由 issue #1211 / MVP-006 实现，但尚未合并；该条件
继续保持开放，必须等待 MVP-006 合并且 MVP-007 控制中心也完成真实消费后共同关闭。

### M3：双角色最小界面（机构工作台待审查/合并，控制中心待完成）

- 机构工作台交付策略/回测摘要和运行中的账户、仓位、订单、成交、风险只读页面。
- 控制中心交付单节点注册、生命周期、健康、日志、指标和事件页面。
- 通过共享状态链接在业务影响与技术根因之间跳转，不混合两个门户的一级导航。

当前进度：MVP-006 已实现机构工作台的策略/回测身份、四轴状态、账户、持仓、订单、
成交、风险、生命周期、来源与边界视图，当前待审查与合并；控制中心页面仍由
MVP-007 承接。
