...
This guide covers the architectural principles and structure of NTPRO, a Rust-only
release workspace derived from NautilusTrader:
...
:::note
NTPRO's current product surface is Rust-only. References to Python, PyO3, Cython,
wheel packaging, or the upstream Python package are legacy or migration context unless
explicitly marked as a supported Rust-only path.
...
The NTPRO codebase is a Rust workspace for composing trading systems and a set of
default system implementations which can operate in various
[environment contexts](#environment-contexts).

For the current Rust-only architecture map, see
[Rust-only architecture map](../architecture/rust_only_architecture_map.md).
...
The foundation of the codebase is the `crates` directory, containing the Rust crates
that define the supported NTPRO product runtime and build path.

Legacy upstream Python/Cython package paths have been removed from the NTPRO product
surface. Any remaining Python, PyO3, or Cython references in historical documentation,
migration evidence, or archived upstream context are unsupported for current NTPRO use.
...
flowchart TB
    ...
    subgraph product["Rust product surface"]
    end
...
:::note
Cargo is the supported build path for NTPRO. Legacy Cython and wheel build guidance
belongs to upstream history and migration evidence, not the current product path.
:::

### Type safety

The platform design prioritizes software correctness and deterministic trading behavior.
...
The documentation aims to cover the errors that NTPRO Rust code can raise or return,
and the conditions that trigger them.

:::warning
Legacy upstream Python exception guidance is not part of the current NTPRO Rust-only
product contract.
:::
