...
NTPRO retains FFI memory-contract notes for Rust-native and historical upstream
interop work. Python, PyO3, and Cython references on this page are legacy
context unless a scoped migration task explicitly says otherwise. They are not
current NTPRO product entrypoints.

The workspace exposes several **C-compatible** types so compiled Rust code can
be consumed by native-language boundaries. The most important of these is
`CVec` - a *thin* wrapper around a Rust `Vec<T>` that is passed across the FFI
boundary **by value**.
...
| **2** | Foreign consumer (legacy Python/Cython or C) | Use the data while the `CVec` value is in scope. **Do not modify the fields `ptr`, `len`, `cap`.** |
...
## Legacy capsules created on the Python side

Several upstream Cython routines allocated temporary C buffers with `PyMem_Malloc`, wrap them into a
...
## Legacy capsules created on the Rust side *(PyO3 bindings)*

This PyO3 pattern is retained as upstream history. Do not add new PyO3 product
bindings for NTPRO. Historically, when Rust code pushed a heap-allocated value
into Python and Python became the final owner, it used
`PyCapsule::new_with_destructor` so Python knew how to free the allocation once
the capsule became unreachable. The closure/destructor was responsible for
...
2. Mark that wrapper `#[repr(transparent)]` over `CVec`, or use `#[repr(C)]` with
   `CVec` as the first field, before casting capsule pointers back to `*mut CVec`.
3. Give the capsule a stable, explicit name, such as `nautilus.DataFFI.CVec`.
   Do not use the default unnamed capsule for this pattern.
...
6. Call that drop function only for capsules created as `CVec` batches. Never pass a
...
8. Reset the stored `CVec` metadata to `CVec::empty()` before calling `Vec::from_raw_parts`,
   so cleanup paths can call the drop function more than once without double-freeing.
...
3. A legacy upstream *Python/Cython* binding had to guarantee that `*_drop` was
    invoked exactly once. Two historical approaches existed:

    • **Preferred for upstream PyO3 code**: Wrap the pointer in a `PyCapsule` created with
...
New NTPRO product code should not add a Python/PyO3/Cython ownership path. If a
future scoped migration task needs one for historical analysis, it must record
the reason and use destructor-backed ownership rather than manual unchecked
freeing.
