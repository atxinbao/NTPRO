...
| Data/catalog workflows | Python catalog, wrangler, loader, Databento/Tardis examples, docs data guides | `nautilus data inspect/validate/load` contract and `examples/rust/data/README.md` | Contract/help-level only; loader/catalog run path remains blocked. | Do not remove Python data tooling until Rust catalog inspect/load path is implemented and scoped. |
| Config validation workflows | Python config objects and example config scripts | `nautilus config validate --kind <backtest|sandbox|live|data>` contract | Contract/help-level only; shared parser/validator remains blocked. | Do not remove Python config workflows until shared Rust config models validate real examples. |
