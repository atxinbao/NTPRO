...
| `IGN-RISK-001` through `IGN-RISK-006` | `SCOPED_OUT_FOR_V04`; historical v0.4 decision. `IGN-RISK-001` and `IGN-RISK-002` were later restored by PAR-003; `IGN-RISK-003` through `IGN-RISK-005` were restored at the current strategy routing owner by PAR-004; the account-balance item remains open. | v0.4 proves one halted-state Binance sandbox rejection through `V04-009`; it did not claim order-list reducing, emulator routing, or account-balance tracking. |
