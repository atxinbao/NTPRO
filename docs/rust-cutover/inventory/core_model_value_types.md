...
- RCORE-003 closes the raw fixed-point spurious-bit gap for
  `from_raw_checked` on `Price`, `Quantity`, and `Money`. The lower-level
  panicking `from_raw` constructors remain a documented compatibility exception
  because existing arithmetic and property tests use arbitrary internal raw
  units for exact max/min and overflow-boundary coverage.
...
| Price | `crates/model/src/types/price.rs` | `Price` has checked constructors, raw constructors, serde, arithmetic, and high/standard precision branches. | RCORE-003 enforces spurious raw scale validation in `Price::from_raw_checked`; `Price::from_raw` remains a low-level internal raw compatibility entry point. |
| Quantity | `crates/model/src/types/quantity.rs` | `Quantity` has checked/non-zero constructors, raw constructors, serde, arithmetic, and DeFi conversion paths. | RCORE-003 enforces spurious raw scale validation in `Quantity::from_raw_checked`; `Quantity::from_raw` remains a low-level internal raw compatibility entry point. |
| Money | `crates/model/src/types/money.rs` | `Money` carries currency precision and has checked/raw/decimal/mantissa constructors plus serde and property tests. | RCORE-003 enforces spurious raw scale validation in `Money::from_raw_checked`; `Money::from_raw` remains a low-level internal raw compatibility entry point. |
...
| CMVT-004 | Raw fixed-point spurious-bit validation is enforced for checked Rust constructors, with a documented `from_raw` compatibility exception. | RCORE-003 removes the TODO markers in `crates/model/src/types/price.rs`, `quantity.rs`, and `money.rs`; `crates/model/tests/value_type_gate.rs` covers invalid raw scale rejection through `from_raw_checked`. | Public checked constructors now reject scale-mismatched raw values. Low-level `from_raw` still accepts arbitrary in-range internal raw units for existing max/min and property-test coverage. | Closed for checked constructors in RCORE-003; any future strict `from_raw` migration needs a separate public API/migration gate. |
...
| CMVT-007 | Checked and panicking constructors are mixed across Rust, FFI, and Python boundary expectations. | `UnixNanos::from_millis`, `UUID4::from`, `StackStr::from_c_ptr`, `Price::new`, `Quantity::new`, `Money::new`, and RCORE-003 raw-constructor evidence. | Runtime/product callers need an explicit contract for checked vs panicking constructors before broad Rust-only API exposure. | Partially classified in RCORE-003: checked raw constructors are strict; panicking `from_raw` remains a low-level compatibility entry point. Broader API policy remains release-gated. |
