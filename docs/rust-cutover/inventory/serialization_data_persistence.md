...
- `nautilus-serialization` provides Arrow, display Arrow, SBE, and Python Arrow
  bindings.
...
- SBE, Arrow, Parquet, Feather, DataFusion, object-store, and custom data parity
  gaps.
...
- Cap'n Proto schema/generated Rust files were removed in RREM-015 and are no
  longer a supported serialization surface.
...
| Cap'n Proto | Removed in RREM-015. | The optional `capnp` feature, schemas, generated Rust files, conversions, tests, benches, scripts, and CI hooks are removed. | No release-stability decision is needed because this format is no longer part of the NTPRO product surface. |
...
| SDP-003 | SBE wire-format stability is explicitly not release-gated. | `crates/serialization/README.md` and `src/lib.rs` warn SBE schemas/wire formats are not stable. Cap'n Proto was removed in RREM-015. | Rust-only release cannot rely on SBE as a stable external wire format without a schema compatibility decision. | RTRACE schema/golden fixtures; RCORE-006 stability decision. |
...
- No SBE, Arrow, Parquet, Feather, DataFusion, or object-store behavior changes
  are allowed here.
...
| SDP-003 | Scoped deferral for SBE. Cap'n Proto was removed in RREM-015. | README/module warnings still state SBE schema and wire-format instability. RCORE-006 did not change schemas or generated code. | RTRACE schema/golden-trace decision before release-stable SBE claims. |
...
- Rust runtime parity for local Arrow/Parquet catalog precision,
  ...
  persistence chain.
- Python/PyO3/Cython removal, SBE release stability, remote object-store
  support, Feather rotation release behavior, unsafe audit, and
