...
hosted release gate, tag object and peeled commit, release body hash/source
match, issue closeout, milestone closeout, and generated-evidence policy.

Plain Chinese summary: 本任务只把 v0.28.0 已发布后的真实 GitHub closeout 事实写回源码树。
它确认 release 已公开、release gate 已成功、body hash 和源码 release notes 一致、v0.28.0
milestone 已关闭，并且 `release-publication-evidence/` 仍只是生成证据，不是唯一证明。
