...
## Tag-Gate Corrective Note

The first `ntpro-rust-only-v0.25.1` tag gate run failed because
`verify_v25_1_post_release_gate_split.sh` still required at least one live V251
issue to remain open. That condition is valid before #811 closes, but not in
the tag gate after #811 and the v0.25.1 milestone are closed. The gate now
accepts the all-closed V251 issue set and continues to block v0.26.0 through
the missing v0.25.1 release-publication evidence.
