...
`v0.27.0` 是当前正式公开发布点；它是 Product Operations Runtime Integration
Foundation。它建立在 v0.26.1 基础上，收口 v0.26.1 dependency proof、
product operations boundary、external identity/permission foundation、
persistent audit storage foundation、deployment/upgrade/rollback orchestration
foundation、long-run telemetry/SLO evidence、Admin Workbench runtime state
bridge、runtime integration fail-closed hardening、v27 release gates 和 strict
provenance。
...
或 Admin 下单/审批/撤单/重试/submit/replace/amend/flatten/remediation/order-ticket 控件。
`v0.28.0` 是下一能力轨占位。
...
v0.27.1 is reserved for future patch hardening only. It must not expand beyond
the v0.27.0 product operations runtime integration foundation boundary unless a later
...
v0.28.0 is the next gated capability track after the V270 release proof. It does
...
approval consumption, or Dashboard/Admin operation controls from v0.27.0.
