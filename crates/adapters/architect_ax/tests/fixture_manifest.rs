...
//! Fixture manifest checks for the Architect AX Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 5,
        "expected Architect AX HTTP and WebSocket fixture groups"
    );

    let required_surfaces = [
        "architect_ax_http_auth_account_lifecycle",
        "architect_ax_http_market_data_parser",
        "architect_ax_http_execution_lifecycle",
        "architect_ax_ws_market_data_levels",
        "architect_ax_ws_order_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("AX-ADP-"), "unexpected blocker id: {id}");
...
#[test]
fn rust_fixture_manifest_pins_architect_ax_boundary_decisions() {
    let manifest_text =
        fs::read_to_string(manifest_path()).expect("rust fixture manifest must be readable");
    let manifest: Value =
        serde_json::from_str(&manifest_text).expect("rust fixture manifest must be valid JSON");
    ...
    for expected in [
        ("AX-ADP-002", "perpetual-futures"),
        ("AX-ADP-003", "L1 downgrade"),
        ("AX-ADP-004", "GTD"),
        ("AX-ADP-005", "token auth"),
        ("AX-ADP-006", "removal gate"),
    ] {
        let entry = blockers
            .iter()
...
fn rust_adapter_gap_closure_is_complete_and_scoped() {
    ...
    assert_eq!(manifest["adapter"], "architect_ax");
    assert_eq!(
    ...
    assert_eq!(decisions.len(), 6, "all Architect AX gaps must be resolved");
    ...
    for decision in decisions {
        ...
        assert!(gap_id.starts_with("AX-ADP-"), "unexpected gap id {gap_id}");
