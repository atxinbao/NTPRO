...
//! Fixture manifest checks for the dYdX Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 6,
        "expected dYdX parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "dydx_http_instrument_parser",
        "dydx_http_market_data_parser",
        "dydx_http_account_execution_reports",
        "dydx_ws_market_data_parser",
        "dydx_ws_subaccount_lifecycle",
        "dydx_block_grpc_operational_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("DYDX-ADP-"), "unexpected blocker id: {id}");
    ...
    for entry in closure {
        ...
        assert!(id.starts_with("DYDX-ADP-"), "unexpected closure id: {id}");
