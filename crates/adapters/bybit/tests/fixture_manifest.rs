...
//! Fixture manifest checks for the Bybit Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 5,
        "expected Bybit parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "bybit_http_instrument_parser",
        "bybit_http_market_data_parser",
        "bybit_http_execution_lifecycle",
        "bybit_ws_market_data_parser",
        "bybit_ws_account_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("BYB-ADP-"), "unexpected blocker id: {id}");
