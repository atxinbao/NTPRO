...
//! Fixture manifest checks for the Deribit Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 6,
        "expected Deribit parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "deribit_http_instrument_parser",
        "deribit_http_market_data_parser",
        "deribit_http_account_lifecycle",
        "deribit_ws_market_data_parser",
        "deribit_ws_execution_lifecycle",
        "deribit_rpc_session_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("DER-ADP-"), "unexpected blocker id: {id}");
