...
//! Fixture manifest checks for the Databento Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert_eq!(manifest["task_id"], "RADP-014");
    assert_eq!(manifest["inventory_task"], "RADP-013");
    ...
    assert!(
        groups.len() >= 5,
        "expected Databento parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "databento_dbn_instrument_parser",
        "databento_dbn_book_parser",
        "databento_dbn_trades_bars_parser",
        "databento_dbn_custom_data_parser",
        "databento_live_feed_handler_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("DBN-ADP-"), "unexpected blocker id: {id}");
        assert!(blocker_ids.insert(id), "duplicate blocker id {id}");
        assert_eq!(blocker["inventory_task"], "RADP-013");
        assert_eq!(blocker["owner_task"], "RADP-015");
        assert!(
    ...
    for expected in [
        ...
        assert!(
            ...
            "missing RADP-013 blocker entry for {expected}"
        );
    }
}
...
fn rust_adapter_gap_closure_is_complete_and_scoped() {
    ...
    assert_eq!(manifest["schema_version"], 1);
    assert_eq!(manifest["task_id"], "RADP-015");
    assert_eq!(manifest["inventory_task"], "RADP-013");
    assert_eq!(manifest["fixture_task"], "RADP-014");
    assert_eq!(manifest["adapter"], "databento");
    assert_eq!(
    ...
    assert_eq!(decisions.len(), 7, "all Databento gaps must be resolved");
    ...
    for decision in decisions {
        ...
        assert!(gap_id.starts_with("DBN-ADP-"), "unexpected gap id {gap_id}");
        ...
        assert_ne!(status, "open", "RADP-015 must not leave open gap {gap_id}");
    ...
    for expected in [
        ...
        assert!(
            ...
            "missing RADP-015 closure decision for {expected}"
        );
    }
}
