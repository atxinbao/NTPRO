...
//! Fixture manifest checks for the Coinbase Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert_eq!(manifest["task_id"], "RADP-008");
    assert_eq!(manifest["inventory_task"], "RADP-007");
    ...
    assert!(
        groups.len() >= 6,
        "expected Coinbase parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "coinbase_http_instrument_parser",
        "coinbase_http_market_data_parser",
        "coinbase_http_execution_lifecycle",
        "coinbase_ws_market_data_parser",
        "coinbase_ws_user_lifecycle",
        "coinbase_derivatives_polling",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("CB-ADP-"), "unexpected blocker id: {id}");
        assert!(blocker_ids.insert(id), "duplicate blocker id {id}");
        assert_eq!(blocker["inventory_task"], "RADP-007");
        assert_eq!(blocker["owner_task"], "RADP-009");
        assert!(
    ...
    for expected in [
        ...
        assert!(
            ...
            "missing RADP-007 blocker entry for {expected}"
        );
    }
}
