...
//! Fixture manifest checks for the Kraken Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 5,
        "expected Kraken parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "kraken_spot_http_parser",
        "kraken_futures_http_parser",
        "kraken_spot_ws_parser",
        "kraken_futures_ws_parser",
        "kraken_execution_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("KRK-ADP-"), "unexpected blocker id: {id}");
