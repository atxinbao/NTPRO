...
//! Fixture manifest checks for the Tardis Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 5,
        "expected Tardis parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "tardis_http_instrument_parser",
        "tardis_machine_market_data_parser",
        "tardis_csv_stream_parser",
        "tardis_data_client_lifecycle",
        "tardis_replay_output_scope",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("TDS-ADP-"), "unexpected blocker id: {id}");
...
fn rust_adapter_gap_closure_is_complete_and_scoped() {
    ...
    assert_eq!(manifest["adapter"], "tardis");
    assert_eq!(
    ...
    assert_eq!(decisions.len(), 7, "all Tardis gaps must be resolved");
    ...
    for decision in decisions {
        ...
        assert!(gap_id.starts_with("TDS-ADP-"), "unexpected gap id {gap_id}");
