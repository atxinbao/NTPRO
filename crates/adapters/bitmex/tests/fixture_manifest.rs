...
//! Fixture manifest checks for the BitMEX Rust adapter parity gate.
...
fn rust_fixture_manifest_is_complete_and_resolvable() {
    ...
    assert!(
        groups.len() >= 6,
        "expected BitMEX parser and lifecycle fixture groups"
    );

    let required_surfaces = [
        "bitmex_http_instrument_parser",
        "bitmex_http_market_data_parser",
        "bitmex_http_execution_lifecycle",
        "bitmex_ws_market_data_parser",
        "bitmex_ws_private_lifecycle",
        "bitmex_operational_lifecycle",
    ];
    ...
    for blocker in blockers {
        ...
        assert!(id.starts_with("BMX-ADP-"), "unexpected blocker id: {id}");
