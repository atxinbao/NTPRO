...
impl EncodeToRecordBatch for PerpetualContract {
    fn encode_batch(
        ...
        for perp in data {
            ...
            asset_class_builder.append_value(perp.asset_class);

            if let Some(base_cur) = perp.base_currency {
                base_currency_builder.append_value(base_cur.to_string());
            } else {
                base_currency_builder.append_null();
            }

            quote_currency_builder.append_value(perp.quote_currency.to_string());
            settlement_currency_builder.append_value(perp.settlement_currency.to_string());
            is_inverse_builder.append_value(perp.is_inverse);
            price_precision_builder.append_value(perp.price_precision);
            size_precision_builder.append_value(perp.size_precision);
            price_increment_builder.append_value(perp.price_increment.to_string());
            size_increment_builder.append_value(perp.size_increment.to_string());
            multiplier_builder.append_value(perp.multiplier.to_string());
            lot_size_builder.append_value(perp.lot_size.to_string());

            if let Some(max_qty) = perp.max_quantity {
                max_quantity_builder.append_value(max_qty.to_string());
            } else {
                max_quantity_builder.append_null();
            }

            if let Some(min_qty) = perp.min_quantity {
                min_quantity_builder.append_value(min_qty.to_string());
            } else {
                min_quantity_builder.append_null();
            }

            if let Some(max_not) = perp.max_notional {
                max_notional_builder.append_value(max_not.to_string());
            } else {
                max_notional_builder.append_null();
            }

            if let Some(min_not) = perp.min_notional {
                min_notional_builder.append_value(min_not.to_string());
            } else {
                min_notional_builder.append_null();
            }

            if let Some(max_p) = perp.max_price {
                max_price_builder.append_value(max_p.to_string());
            } else {
                max_price_builder.append_null();
            }

            if let Some(min_p) = perp.min_price {
            ...
            ts_event_builder.append_value(perp.ts_event.as_u64());
...
pub fn decode_perpetual_contract_batch(
    ...
    let base_currency_values = cols
        .get(4)
        .ok_or_else(|| EncodingError::MissingColumn("base_currency", 4))?;
    let quote_currency_values =
        extract_column::<StringArray>(cols, "quote_currency", 5, DataType::Utf8)?;
    let settlement_currency_values =
        extract_column::<StringArray>(cols, "settlement_currency", 6, DataType::Utf8)?;
    let is_inverse_values =
        extract_column::<BooleanArray>(cols, "is_inverse", 7, DataType::Boolean)?;
    let price_precision_values =
        extract_column::<UInt8Array>(cols, "price_precision", 8, DataType::UInt8)?;
    let size_precision_values =
        extract_column::<UInt8Array>(cols, "size_precision", 9, DataType::UInt8)?;
    let price_increment_values =
        extract_column::<StringArray>(cols, "price_increment", 10, DataType::Utf8)?;
    ...
    let lot_size_values = extract_column::<StringArray>(cols, "lot_size", 13, DataType::Utf8)?;
    let max_quantity_values = cols
        .get(14)
        .ok_or_else(|| EncodingError::MissingColumn("max_quantity", 14))?;
    let min_quantity_values = cols
        .get(15)
        .ok_or_else(|| EncodingError::MissingColumn("min_quantity", 15))?;
    let max_notional_values = cols
        .get(16)
        .ok_or_else(|| EncodingError::MissingColumn("max_notional", 16))?;
    let min_notional_values = cols
        .get(17)
        .ok_or_else(|| EncodingError::MissingColumn("min_notional", 17))?;
    let max_price_values = cols
        .get(18)
        .ok_or_else(|| EncodingError::MissingColumn("max_price", 18))?;
    let min_price_values = cols
        .get(19)
        .ok_or_else(|| EncodingError::MissingColumn("min_price", 19))?;
