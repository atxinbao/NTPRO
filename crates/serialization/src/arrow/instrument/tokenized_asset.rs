...
impl EncodeToRecordBatch for TokenizedAsset {
    fn encode_batch(
        ...
        for ta in data {
            id_builder.append_value(ta.id.to_string());
            raw_symbol_builder.append_value(ta.raw_symbol);
            asset_class_builder.append_value(ta.asset_class.as_ref());
            base_currency_builder.append_value(ta.base_currency.to_string());
            quote_currency_builder.append_value(ta.quote_currency.to_string());

            if let Some(isin) = ta.isin {
                isin_builder.append_value(isin.as_str());
            } else {
                isin_builder.append_null();
            }

            price_precision_builder.append_value(ta.price_precision);
            size_precision_builder.append_value(ta.size_precision);
            price_increment_builder.append_value(ta.price_increment.to_string());
            size_increment_builder.append_value(ta.size_increment.to_string());
            multiplier_builder.append_value(ta.multiplier.to_string());

            if let Some(lot_size) = ta.lot_size {
                lot_size_builder.append_value(lot_size.to_string());
            } else {
                lot_size_builder.append_null();
            }

            if let Some(max_qty) = ta.max_quantity {
                max_quantity_builder.append_value(max_qty.to_string());
            } else {
                max_quantity_builder.append_null();
            }

            if let Some(min_qty) = ta.min_quantity {
                min_quantity_builder.append_value(min_qty.to_string());
            } else {
                min_quantity_builder.append_null();
            }

            if let Some(max_not) = ta.max_notional {
                max_notional_builder.append_value(max_not.to_string());
            } else {
                max_notional_builder.append_null();
            }

            if let Some(min_not) = ta.min_notional {
                min_notional_builder.append_value(min_not.to_string());
            } else {
                min_notional_builder.append_null();
            }

            if let Some(max_p) = ta.max_price {
                max_price_builder.append_value(max_p.to_string());
            } else {
                max_price_builder.append_null();
            }

            if let Some(min_p) = ta.min_price {
                min_price_builder.append_value(min_p.to_string());
            } else {
                min_price_builder.append_null();
            }

            margin_init_builder.append_value(ta.margin_init.to_string());
            margin_maint_builder.append_value(ta.margin_maint.to_string());
            maker_fee_builder.append_value(ta.maker_fee.to_string());
            taker_fee_builder.append_value(ta.taker_fee.to_string());

            // Encode info dict as JSON bytes (using serde_json::to_vec)
            if let Some(ref info) = ta.info {
            ...
            ts_event_builder.append_value(ta.ts_event.as_u64());
            ts_init_builder.append_value(ta.ts_init.as_u64());
        }
...
pub fn decode_tokenized_asset_batch(
    #[allow(unused)] metadata: &HashMap<String, String>,
    record_batch: &RecordBatch,
) -> Result<Vec<TokenizedAsset>, EncodingError> {
    ...
    let lot_size_values = cols
        .get(11)
        .ok_or_else(|| EncodingError::MissingColumn("lot_size", 11))?;
    ...
    let max_notional_values = cols
        .get(14)
        .ok_or_else(|| EncodingError::MissingColumn("max_notional", 14))?;
    let min_notional_values = cols
        .get(15)
        .ok_or_else(|| EncodingError::MissingColumn("min_notional", 15))?;
    let max_price_values = cols
        .get(16)
        .ok_or_else(|| EncodingError::MissingColumn("max_price", 16))?;
    let min_price_values = cols
        .get(17)
        .ok_or_else(|| EncodingError::MissingColumn("min_price", 17))?;
    ...
    let info_values = cols
        .get(22)
        .ok_or_else(|| EncodingError::MissingColumn("info", 22))?;
    let ts_event_values = extract_column::<UInt64Array>(cols, "ts_event", 23, DataType::UInt64)?;
    let ts_init_values = extract_column::<UInt64Array>(cols, "ts_init", 24, DataType::UInt64)?;
    ...
    for i in 0..num_rows {
        ...
        let tokenized_asset = TokenizedAsset::new(
            id,
            raw_symbol,
            asset_class,
            base_currency,
