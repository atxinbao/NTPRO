...
impl EncodeToRecordBatch for CurrencyPair {
    fn encode_batch(
        ...
        for cp in data {
            ...
            if let Some(lot_size) = cp.lot_size {
...
pub fn decode_currency_pair_batch(
    ...
    // Read precisions from data columns (they're in the schema)
    ...
    // Note: We read precisions from the data columns, not metadata
    ...
    let multiplier_values = extract_column::<StringArray>(cols, "multiplier", 8, DataType::Utf8)?;
